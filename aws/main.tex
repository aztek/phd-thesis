...
% \begin{document}
% \title{Checking Network Reachability Properties\\with Complementary Constraint Solvers}
% \author{\IEEEauthorblockN{
% Byron Cook\IEEEauthorrefmark{1}\IEEEauthorrefmark{2},
% Temesghen Kahsai\IEEEauthorrefmark{1},
% Evgenii Kotelnikov\IEEEauthorrefmark{1}\IEEEauthorrefmark{3}, 
% Sean McLaughlin\IEEEauthorrefmark{1}, 
% Pavle Suboti\'{c}\IEEEauthorrefmark{1}\IEEEauthorrefmark{2}
% }
% \IEEEauthorblockA{\IEEEauthorrefmark{1}Amazon Web Services}
% \IEEEauthorblockA{\IEEEauthorrefmark{2}University College London}
% \IEEEauthorblockA{\IEEEauthorrefmark{3}Chalmers University of Technology}
% }
% \maketitle

% \begin{abstract}
% Verification of network configurations is an important and challenging task, often tackled with constraint solving and automated reasoning. In this work we compare several constraint solvers that we use to check reachability properties of Amazon Virtual Private Cloud (VPC) networks. We model Amazon networks with Horn clauses and check first-order properties of these models using Datalog engines and automated theorem provers. We demonstrate that by using multiple types of constraint solvers we are able to pick the most efficient solver for each given network property. This allows us to increase the efficiency of network verification for Amazon VPC networks.
% \end{abstract}
...
Figure~\ref{ex:vpc} shows an example of a VPC network that consists of two subnets ``Web'' and ``Database'' with three network instances in each of them. Each of the subnets is assigned with a route table that configures the ``Web'' subnet to be accessible from the internet and the ``Database'' to not be accessible from the internet. In addition, each of the subnets is assigned with an access control list (ACL) that contains their security access rules. In particular, one of the rules forbids SSH access to the database servers directly or indirectly (via the web servers). \EK{We need to rephrase this paragraph, I think the difference between route tables and ACLs is unclear.}
