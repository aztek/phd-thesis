...
Answering network questions manually might be tedious and error-prone in an industrial-size network. For this reason it is necessary to automate this task with specialized tools. In this work we employ automated theorem provers for first-order logic. To this end, we build static models of VPC networks (Section~\ref{sect:aws/specification}), translate these models the network questions about them to problems in first-order logic (Section~\ref{sect:aws/fol}) and check these problems using finite model builders and saturation-based theorem provers (Section~\ref{sect:aws/fol-provers}). We use these two types of provers to answer both types of questions more efficiently. As we show in Section~\ref{sect:aws/fol-provers}, finite model builders can more efficiently answer list questions and saturation-based theorem prover can more efficiently answer boolean questions.

\EK{Experiments?}

\EK{Mention challenges and future work?}

\EK{Summary of contributions?}
...
We use formal specifications of VPC networks written in a logic programming style. A network model is a finite set of first-order Horn clauses. We disallow function symbols with positive arity and allow stratified negation. We assume the plain logic programming semantics for these Horn clauses, defined in the standard way (see e.g.~\cite{DBLP:books/sp/Lloyd87}). In particular, we make the closed-world assumption and treat negation as failure. In addition, our network models use the theory of bit vectors to describe ports, IPv4 addresses, subnet masks and CIDR (Classless Inter-Domain Routing) addresses.
...
\EK{We use 16-bit vectors to represent ports and port ranges. Port ranges are represented using the type $\type{portRange}$ and the predicate $\pred{portRange}$ of the type $\type{portRange}\times\type{bits16}\times\type{bits16}$. The port range 0--1023 is represented using the constant $\const{portRange}_{0--1023}$ and the fact $\pred{portRange}(\const{portRange}_{0--1023}, \const{bits16}_0, \const{bits16}_{1023})$. Our specification contains the following definition of the predicate $\pred{portRangeOverlap}$ of the type $\type{portRange}\times\type{portRange}$ that holds when two port ranges overlap.}
\begin{equation*}
  \begin{aligned}
...
The network model uses two theories of bit vectors: 16-bit vectors to represent port numbers (as integers between 0 and 65535) and 32-bit vectors to represent IPv4 addresses, subnet masks and CIDR addresses. These two theories use different predicates, therefore we translate 16-bit and 32-bit vectors differently.

Our early experiments with translating the theory of bit vectors revealed that the quality of this translation crucially affects the performance of Vampire. This led us to develop and compare multiple translations. We translate 16-bit vectors to integers and 32-bit vectors to tuples and arrays. We also describe a technique that eliminates theories from our problems by precomputing the results of theory operations over ports, IPv4 addresses, subnet masks and CIDR addresses.

\subsubsection*{16-bit vectors}
%\paragraph*{16-bit vectors as integers}%\label{sect:fol/theories/16}
The network model uses the type $\type{bits16}$ and predicates $\pred{bits16}_{\le}$ (``less or equal'') and $\pred{bits16}_{\ge}$ (``greater or equal''). We translate
\begin{enumerate*}[label=(\roman*)]
...
\subsubsection*{32-bit vectors}
The network model uses the type $\type{bits32}$ and the predicate $\pred{bits32}_\wedge$ (bitwise conjunction).

%Our translation of the theory of 32-bit vectors is not the only possible one. For example, one can use the theory of boolean arrays instead.

%We translate 32-bit vectors using theory of tuples, supported by Vampire. To this end, we

\paragraph*{Tuples}%\label{sect:fol/theories/32}
We translate
\begin{enumerate*}[label=(\roman*)]
...
We translate
\begin{enumerate*}[label=(\roman*)]
  \item the type $\type{bits32}$ to the array sort $\mathit{array}(\mathbb{Z},\mathit{bool})$,
  \item each of the 32-bit vector constant to a fresh constant $v$ of this sort, defined using the axiom $\mathit{select}(v,1)=b_1\wedge\ldots\wedge\mathit{select}(v, 32)=b_{32},$ where each of $b_1,\ldots,b_{32}$ is either $\mathit{true}$ or $\mathit{false}$, and
  \item predicate $\pred{bits32}_\wedge$ to an axiom that defines the bitwise conjunction for arrays of booleans.
\end{enumerate*}

We found that the translation that uses arrays of booleans produces formulas that are harder for \vampire than the translation that uses tuples. We explain it by the fact that in the latter translation a 32-bit vector is represented as a single term and not 32 literals. This representation allows Vampire to infer bitwise conjunctions faster.
...
For example, consider a network that only uses port ranges 0--65535, 22--22, 443--443, 5432--5432, 80--80, 80--81 in the configuration of any of its components. Recall the definition of the $\pred{portRangeOverlap}$ predicate from Section~\ref{sect:aws/reachability/properties}.

12 facts of the form $\pred{portRangeOverlap}(\const{portRange}_X,\const{portRange}_Y)$, where $X$ and $Y$ are either of 0--65535, 22--22, 443--443, 5432--5432, 80--80 or 80--81.
...
\begin{equation*}
\begin{aligned}
&\pred{portRangeOverlap}(\mathit{Range},\mathir{Range}).\\
&\pred{portRangeOverlap}(\mathit{R}_1,\mathir{R}_2)\leftarrow\pred{portRangeOverlap}(\mathit{R}_2,\mathir{R}_1).\\
...
This paper presents a work in progress and we did not fully address some of the challenges that we faced. We discuss these challenges in this section. We believe that similar challenges are experienced in other practical applications of automated reasoning in first-order logic.
...
Our formalization of VPC networks uses large finite domains to describe the topology of a network and the semantics of its components. While finite domains is good news for finite model builders, saturation-based theorem provers are known to have hard time reasoning with them. Domain closure axioms $(\forall x:\tau)(x=c_1 \vee\ldots\vee x=c_n)$ for large values of $n$ result in long clauses that degrade performance of the prover. A technique for efficient superposition theorem proving with these axioms is discussed in \cite{HillenbrandWeidenbach13}, but this technique is not implemented in any modern theorem prover.
...
\EK{We need to run experiments.}

\paragraph{Comparison of theorem provers}
In this work we only used Vampire. \EK{but would be interesting to compare other provers, or even use a portfolio of provers instead of just one.}
