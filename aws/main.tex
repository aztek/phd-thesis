...
%
Despite the vast body of available tools, the problem of practical verification of enterprise networks remains a challenge. The size and intricacy of modern industrial-sized networks results in very difficult constraint satisfaction problems. While most network verification tools depend on a single constraint solving paradigm, we find that no single solver achieves best performance on all verification problems. We are interested in checking properties of VPC networks using complementary constraint solvers. Our ultimate goal is to build a portfolio of constraint solvers that leverages the advantages of different types of solvers on different types of properties. In an effort towards this goal in this work we describe an approach to network verification that relies on automated reasoning for first-order logic. Our plan is to integrate this approach in our ongoing work on the portfolio of solvers.

% In an effort towards this goal in this work we investigate whether certain network verification problems can be solved more efficiently using automated reasoning tools in first-order logic, such as saturation-based theorem provers and finite model builders, than other solvers. The intuition

% First-order logic with theories provides a convenient framework for expressing both the configuration of a network and its properties.

% In an effort towards this goal in this work we investigate whether certain network verification problems can be solved more efficiently using automated reasoning tools in first-order logic than other solvers. Our motivation for that is twofold. On one hand, first-order logic with theories provides a convenient framework for expressing both the configuration of a network and its properties. Whereas other paradigms might rely on translations, first-order logic allows for more direct encodings .

% On the other hand, the continuous advances in automated reasoning have made
% theorem provers into powerful tools capable of efficiently solving problems coming from real
% world domains.

% In an effort towards this goal in this work we describe an approach to network verification that relies on automated reasoning in first-order logic. Our motivation for investigating this area . First-order logic is highly expressive for encoding both configurations and properties of networks. 

% Our plan is to integrate this approach in our ongoing work on the portfolio of solvers. 

% In addition, continuous advances in automated reasoning in first-order logic have made theorem provers into powerful tools capable of efficiently solving problems coming from real world domains.
...
\begin{table}
  ...
  \begin{tabular}{lll}
    \hline
    Output of a theorem prover & Yes-no question & List question \\
    \hline
...
Bj\o{}rner et al.~\cite{DBLP:conf/icdcit/BjornerJ15} checked properties of ACLs, routing tables and Border Gateway Protocol policies in Microsoft Azure networks using SMT solver Z3~\cite{Z3}. To that end \cite{DBLP:conf/icdcit/BjornerJ15} implemented a translation of network properties to quantifier-free logical formulas over bit vectors that is aware of the semantics of network components. In comparison, our translation encodes the complete semantics of a VPC network in the quantified problem solved by the theorem prover. We argue that our approach is more flexible at the expense of having to deal with more difficult problems.

Weidenbach~\cite{Weidenbach99} and Goubault-Larrecq~\cite{DBLP:journals/jcs/Goubault-Larrecq10} used finite model building for first-order logic to check unreachability in security protocols.

\EK{
  Weidenbach~\cite{Weidenbach99} checked properties of the Neuman-Stubblebine key exchange protocol by expressing them as sets of monadic Horn clauses. Similarly to this work, \cite{Weidenbach99} encodes reachability as a problem in first-order logic. Properties of this protocol were verified using finite saturation in the Spass theorem prover thanks to decidability of the monadic Horn clauses. It is
}
