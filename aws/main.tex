...
Figure~\ref{ex:vpc} shows an example of a network that consists of two subnets ``Web'' and ``Database'' with three network instances in each of them. Each of the subnets is assigned with a route table that configures the ``Web'' subnet to be accessible from the internet and the ``Database'' to not be accessible from the internet. In addition, each of the subnets is assigned with an access control list (ACL) that contains their security access rules. In particular, one of the rules forbids SSH access to the database servers directly or indirectly (via the web servers). \EK{We need to rephrase this paragraph, I think the difference between route tables and ACLs is unclear.}
