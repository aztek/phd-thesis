...
\begin{enumerate*}[label=(\roman*)]
  ...
  \item predicate $\pred{bits32}_\wedge$ to a predicate defined by the axiom of bitwise conjunction for boolean tuples.
  \item predicate $\pred{=}_\type{bits16}$ to the standard equality.
\end{enumerate*}
...
\begin{enumerate*}[label=(\roman*)]
  ...
  \item each of the 32-bit vector constant to a fresh constant $v$ of this sort, defined using the axiom $\mathit{select}(v,1)=b_1\wedge\ldots\wedge\allowbreak\mathit{select}(v, 32)=b_{32},$ where each of $b_1,\ldots,b_{32}$ is either $\mathit{true}$ or $\mathit{false}$, and
  \item predicate $\pred{bits32}_\wedge$ to a predicate defined by the axiom of bitwise conjunction for arrays of booleans.
  \item predicate $\pred{=}_\type{bits16}$ to a predicate defined by the axiom of equality for 32-element arrays of booleans.
\end{enumerate*}
