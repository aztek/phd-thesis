...
\def\paperSevenContentsTitle{Checking Network Reachability Properties with\\Automated Reasoning in First-Order Logic}
\def\paperSevenChapterTitle{Checking Network Reachability\\Properties with Automated Reasoning\\in First-Order Logic}
\def\paperSevenAuthors{Evgenii~Kotelnikov and Pavle Suboti\'{c}}
\def\paperSevenAbstract{Verification of computer networks is an important and challenging task, often tackled with constraint solving and automated reasoning. This work presents an approach for checking and discovering reachability properties of virtual private cloud networks using automated reasoning in first-order logic. Reachability properties of a network express whether its configuration allows the network traffic to flow between given nodes of the network. We model networks with Horn clauses and check first-order properties of these models using the Vampire theorem prover.}
\def\paperSevenPublication{A technical report based on a joint work with Byron~Cook, Temesghen Kahsai and Sean~McLaughlin, 2018.}
